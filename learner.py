...
import time
from sample import SDESampling
import utils
...
class Learner:
    ...
    def train(self):
        ...
        while True:
            ...
            #)
            loss, vectorial_loss, t= self.train_step()

            if torch.isnan(loss).any():
                print(f"Detected NaN loss at step {self.step}.")
    ...
    def train_step(self):
        ...
        for i in range(self.args.microbatches): 
            ...
            stacked_audio=noisy_audio.unsqueeze(1)
    
            
            predicted = self.model(stacked_audio, sigma)
